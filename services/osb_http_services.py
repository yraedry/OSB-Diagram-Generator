...
class OsbHttpService:
    ...
    def get_repos(self) -> None:
        ...
        services_repo=[]
        services_allowed = basic_utils.create_list_from_properties(property_config.read_properties('services', 'start_name'))
        request_bitbucket = BitBucketController(property_config.read_properties('bitbucket', 'endpoint'))
    ...
    def get_components_relations(self, repo, components) -> None:
        ...
        proxies_type = self.get_jms_type_proxy(repo, proxy_components)
    ...
    def get_proxy_relations(self, repo, components):
        ...
        for component in components:
            ...
            proxy_components_relations[component] = pipeline_path_name
    ...
    def get_jms_type_proxy(self, repo, component):
        ...
        if proxy_type == "jms":
            jms_type_proxy_dict[component] = basic_utils.delete_with_pattern(pattern, proxy_type)
        else:
            jms_type_proxy_dict[component] = proxy_type
    ...
    def clone_services_local(self, repos) -> None:
        for repo in repos:
            ...
            clone_url = f'{property_config.read_properties("bitbucket", "clone")}/{repo}.git'
            try:
                if not os.path.exists(service_dir):
                    Repo.clone_from(clone_url, service_dir, config=f'https.proxy=https://{property_config.read_properties("proxy", "https")}, x-token-auth:{property_config.read_properties('credential', 'tokenApi')}', 
                                    branch=property_config.read_properties("bitbucket", "branch"), allow_unsafe_options=True)
                else:
